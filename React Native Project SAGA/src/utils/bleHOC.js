...
export const useBLE = (WrappedComponent, serviceUUID) => {
    return class extends React.Component {
        ...
        async handleUpdateValueForCharacteristic({ value, peripheral, characteristic, service }) {
            let temp = value.filter(value => value < 255 && value > 0) // Filter Dummy Bytes
            ...
            if (this.state.peripheral) {
                BleManager.connect(this.state.peripheral.id).then(() => {
                    setTimeout(async () => {
                        ...
                        BleManager.write(this.state.peripheral.id, serviceUUID, inUUID, [...data], data.length).then(() => { console.log("Write OK") }).catch((e) => {
                            this.setState({
                                methodResponse: "ok"
                            })
                        })
