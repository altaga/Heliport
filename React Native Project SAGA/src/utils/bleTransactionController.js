...
class BLEcontrollerTransaction extends Component {
    constructor(props) {
        super(props);
        this.state = {
            scanning: false,
    ...
    handleDiscoverPeripheralT(peripheral) {
        if (!peripheral.name) {
    ...
    async routineThree(peripheral) {
        await BleManager.retrieveServices(peripheral.id).then((info) => console.log(""))
        let res = Buffer.from(this.props.data.transaction, "utf-8")
        await BleManager.write(peripheral.id, "1101", "0001", [...res], res.length).then(async () => {
            console.log("Transaction ok")
        })
        setTimeout(async () => {
            this.props.callbackPeripheralComplete("Transaction ok");
    ...
    componentDidMount() {
        BleManager.enableBluetooth()
            .then(async () => {
                ...
                this.emitterStop = bleManagerEmitter.addListener('BleManagerStopScan', this.handleStopScanT);
                this.emitterUpdate = bleManagerEmitter.addListener('BleManagerDidUpdateValueForCharacteristic', this.handleUpdateValueForCharacteristicT);
                this.emitterDiscover = bleManagerEmitter.addListener('BleManagerDiscoverPeripheral', this.handleDiscoverPeripheralT);
    ...
    componentDidUpdate(prevProps, prevState) {
        if ((this.props.connect !== prevProps.connect) && this.props.connect === true) {
            if (this.props.routine === 2) {
                this.routineTwo(this.props.peripheral)
            }
            if (this.props.routine === 3) {
                this.routineThree(this.props.peripheral)
            }
        }
    }
...
export default BLEcontrollerTransaction;