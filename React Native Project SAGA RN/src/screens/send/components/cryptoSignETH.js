...
class CryptoSignETH extends Component {
  constructor(props) {
    super(props);
    this.state = {
      biometrics: false,
  ...
  async signTransaction() {
    try {
      const session = await EncryptedStorage.getItem("ethPrivateKey");
      const selectedWallet = JSON.parse(
        await AsyncStorage.getItem("selected")
      ).selected;
      const wallet = new ethers.Wallet(
        JSON.parse(session).privateKey[selectedWallet]
      );
      const serializedSignedTx = await wallet.signTransaction(this.props.transaction)
